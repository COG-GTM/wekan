...
if (Meteor.isServer) {
  ...
  async function buildKanboardExport(boardId: string) {
    ...
    const listById: { [key: string]: any } = {};
    lists.forEach((l: any) => { listById[l._id] = l.title; });
    const swById: { [key: string]: any } = {};
    swimlanes.forEach((s: any) => { swById[s._id] = s.title; });
    const labelById: { [key: string]: any } = {};
    (board.labels || []).forEach((l: any) => { labelById[l._id] = l.name; });
    return {
      board: { name: board.title },
      columns: lists.map((l: any) => ({ title: l.title })),
      swimlanes: swimlanes.map((s: any) => ({ name: s.title })),
      tasks: cards.map((c: any) => ({
        title: c.title,
        ...
        tags: (c.labelIds || []).map((id: any) => labelById[id]).filter(Boolean),
      })),
    };
  }
  ...
  WebApp.handlers.get('/api/boards/:boardId/export', async function (req: any, res: any) {
   ...
   */
  // `req`/`res` come from the untyped WebApp.handlers router, hence `any`.
  WebApp.handlers.get('/api/boards/:boardId/export/kanboard', async function (req: any, res: any) {
    const boardId = req.params.boardId;
    const board = await ReactiveCache.getBoard(boardId);
    if (!board) {
      sendJsonResult(res, { code: 404, data: { error: 'Not found' } });
  ...
  // `req`/`res` come from the untyped WebApp.handlers router, hence `any`.
  async function serveExternalExport(req: any, res: any, format: string) {
    const boardId = req.params.boardId;
    const board = await ReactiveCache.getBoard(boardId);
    if (!board) {
      sendJsonResult(res, { code: 404, data: { error: 'Not found' } });
  ...
  EXTERNAL_EXPORT_FORMATS.forEach(format => {
     ...
     */
    // `req`/`res` come from the untyped WebApp.handlers router, hence `any`.
    WebApp.handlers.get(`/api/boards/:boardId/export/${format}`, async function (req: any, res: any) {
   ...
   */
  // `req`/`res` come from the untyped WebApp.handlers router, hence `any`.
  WebApp.handlers.get('/api/boards/:boardId/export/csv', async function (req: any, res: any) {
    const boardId = req.params.boardId;
    let user = null;
    let impersonateDone = false;
    let adminId = null;
