...
runOnServer(function() {
   ...
   */
  WebApp.handlers.get('/api/boards/:boardId/lists/:listId/cards/:cardId/exportPDF', async function (req: WekanWebAppRequest, res: WekanWebAppResponse) {
   ...
   * @param {string} authToken the loginToken
   */
  WebApp.handlers.get('/api/boards/:boardId/exportPDF', async function (req: WekanWebAppRequest, res: WekanWebAppResponse) {
    const boardId = req.params.boardId;
    let user = null;
