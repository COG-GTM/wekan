...
export function icsEventToCard(
  ...
  { boardId, listId, swimlaneId }: IcsCardOptions = {},
) {
 ...
 */
export function icsToCards(icsText: string, opts?: IcsCardOptions) {
  return parseIcs(icsText).map(event => icsEventToCard(event, opts));
}

// A parsed VEVENT block.
interface IcsEvent {
  summary: string;
  description: string;
  start: Date | null;
...
// Destination placement applied to every card produced from an .ics file.
interface IcsCardOptions {
  boardId?: string;
  listId?: string;
  swimlaneId?: string;
}
