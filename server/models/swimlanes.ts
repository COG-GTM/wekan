...
Meteor.methods({
  async ensureDefaultSwimlane(boardId: string) {
...
WebApp.handlers.get('/api/boards/:boardId/swimlanes', async function(req, res) {
  try {
    ...
    sendJsonResult(res, {
      code: 200,
      data: swimlanes.map(function(doc: WekanDocumentField) {
        return {
          _id: doc._id,
          title: doc.title,
        };
...
async function repositionSwimlane(swimlaneId: string, toBoardId: string, position: WekanDocumentField) {
