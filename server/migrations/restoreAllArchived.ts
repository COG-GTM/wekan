...
function getTranslatedString(key: string, fallback: string, options?: object | string) {
  ...
  async executeMigration(boardId: string) {
    try {
      ...
      for (const list of archivedLists) {
        const updateFields: ArchivedListUpdateFields = {
          archived: false,
          updatedAt: new Date()
        };
        ...
        if (!list.swimlaneId) {
          ...
          let targetSwimlane = activeSwimlanes.find((s: any) => !s.archived);
      ...
      for (const card of archivedCards) {
        const updateFields: ArchivedCardUpdateFields = {
          archived: false,
          updatedAt: new Date()
        };
        ...
        if (!card.listId) {
          ...
          let targetList = allLists.find((l: any) => !l.archived);

          if (!targetList) {
            // No active list found, create one
            const defaultSwimlane = allSwimlanes.find((s: any) => !s.archived) || allSwimlanes[0];
        ...
        if (!card.swimlaneId) {
          ...
          if (card.listId || updateFields.listId) {
            const cardList = allLists.find((l: any) => l._id === (updateFields.listId || card.listId));
...
interface ArchivedListUpdateFields {
  archived: boolean;
  updatedAt: Date;
  swimlaneId?: string;
}

interface ArchivedCardUpdateFields {
  archived: boolean;
  updatedAt: Date;
  listId?: string;
  swimlaneId?: string;
