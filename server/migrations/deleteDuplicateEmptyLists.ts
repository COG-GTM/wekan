...
class DeleteDuplicateEmptyListsMigration {
    ...
    try {
      ...
      for (const list of lists) {
        ...
        if (listCards.length === 0) {
          ...
          const duplicateListsWithSameTitle = lists.filter((l: any) =>
            l._id !== list._id &&
            l.title === list.title &&
            l.boardId === boardId
          );
          ...
          for (const duplicateList of duplicateListsWithSameTitle) {
            const duplicateListCards = cards.filter((card: any) => card.listId === duplicateList._id);
  ...
  async convertSharedListsToPerSwimlane(boardId: string) {
    ...
    for (const sharedList of sharedLists) {
      ...
      const listCards = cards.filter((card: any) => card.listId === sharedList._id);

      // Group cards by swimlane
      // Dynamic dictionary keyed by runtime swimlaneId; card values come from
      // the untyped ReactiveCache, hence `any[]`.
      ...
      for (const swimlane of swimlanes) {
        ...
        if (swimlaneCards.length > 0) {
          ...
          const existingList = lists.find((l: any) =>
