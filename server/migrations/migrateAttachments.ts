...
if (Meteor.isServer) {
  ...
  const requireBoardAccess = async (userId: string, boardId: string | undefined) => {
    if (!boardId) {
  ...
  Meteor.methods({
    ...
    async migrateAttachment(attachmentId: string) {
      ...
      try {
        ...
        // Create new attachment using Meteor-Files. getOldAttachmentDataBuffer
        // resolves to a Buffer, which the original code passes straight to File;
        // cast to BlobPart here so File accepts it without altering that runtime
        // behaviour.
        const fileObj = new File([fileData as unknown as BlobPart], oldAttachment.name, {
    ...
    async migrateCardAttachments(cardId: string) {
      ...
      const results: MigrateCardAttachmentsResult = {
        success: 0,
        failed: 0,
        errors: []
      };
...
interface MigrateCardAttachmentsResult {
  success: number;
  failed: number;
